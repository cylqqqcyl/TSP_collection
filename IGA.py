...
class GA(object):
    ...
    def greedy_init(self, dis_mat, num_total, num_city):  # 生成初始种群
        start_index = 0
        result = []
        for i in range(num_total):
    ...
    def ga_cross(self, x, y):
        ...
        best_y = y.copy()
        best_adp = np.inf
        for i in path_list:  # 遍历所有可能的交叉，选最优的
            start, end = 0, i
            ...
            x = list(x)
            y = list(y)
            x_adp = self.compute_pathlen(x,self.dis_mat)
            y_adp = self.compute_pathlen(y,self.dis_mat)
            if x_adp<best_adp:
                best_adp = x_adp
                best_x = x
            if y_adp < best_adp:
